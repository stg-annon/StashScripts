...
def tag_files(group):
	...
	keep_reasons = [s.remove_reason for s in group if s.remove_reason]
	...
	log.info(f"{keep_scene.id} best of:{[s.id for s in group]} {keep_msgs}")

	for scene in group:
		if scene.id == keep_scene.id:
			tag_ids = [stash.find_tag(config.KEEP_TAG_NAME, create=True).get("id")]
			for reason in keep_reasons:
				tag_ids.append(stash.find_tag(f'[Reason: {reason}]', create=True).get('id'))
			stash.update_scenes({
				'ids': [scene.id],
